...
@section('content')
    ...
    <div class="container-xl">
        <form @if(isset($user->id)) action="{{route('users.update',$user->id)}}" @endif method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="card">
                <div class="row g-0">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row g-3">
                                ...
                                <span class="input-icon-addon">
                                  ...
                                  <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /><path d="M6 21v-2a4 4 0 0 1 4 -4h4a4 4 0 0 1 4 4v2" /></svg>
                                        ...
                                        <input type="text" class="form-control" name="username" id="username" @if(isset($user->username)) value="{{$user->username}}" @endif placeholder="entrez le nom d'utilisateur" Required>
                                    </div>
                                </div>
                                <div class="col-md">
                                    <div class="form-label Required">Nom</div>
                                    <input type="text" class="form-control" name="last_name" id="last_name" @if(isset($user->last_name)) value="{{$user->last_name}}" @endif placeholder="entrez le nom" Required>
                                </div>
                                <div class="col-md">
                                    <div class="form-label Required">Prénom</div>
                                    <input type="text" class="form-control" name="first_name" id="first_name" @if(isset($user->first_name)) value="{{$user->first_name}}" @endif placeholder="entrez le prénom" Required>
                                </div>
                            </div>
                            <div class="row g-3">
                                <div class="col-md">
                                    <div class="form-label Required">Adresse</div>
                                    <input type="text" class="form-control" @if(isset($user->address)) value="{{$user->address}}" @endif name="address" id="address" placeholder="entrez l'adresse">
                                </div>
                                <div class="col-md">
                                    <div class="form-label Required">Numéro de téléphone</div>
                                    <input type="text" class="form-control" @if(isset($user->phone_number)) value="{{$user->phone_number}}" @endif name="phone_number" id="phone_number" placeholder="entrez le numéro de téléphone" Required>
                                </div>
                            </div>
                            ...
                            <div>
                                <div class="row g-2">
                                    ...
                                    @
                                            ...
                                            <input type="text" class="form-control" @if(isset($user->email)) value="{{$user->email}}" @endif name="email" id="email" placeholder="entrez l'e-mail" Required>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <label class="form-check form-switch form-switch-lg Required">
                                    <input class="form-check-input" type="checkbox" name="status" id="status" @if(isset($user->email)) @if($user->status==1) checked @endif @endif>
                                    ...
                                    <select class="form-select" id="role_name" name="role_name[]" data-placeholder="Choose anything" multiple required>
                                        ...
                                        @if(isset($roles))
                                            @foreach($roles as $role)
                                                <option value="{{$role->name}}" @if(in_array($role->name,array_values($user->role_name))) selected @endif>{{$role->name}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
